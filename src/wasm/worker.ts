...
import { optimize, specBuildCreasePattern, exportV5 } from './engine';

export type WorkerRequest =
  | { id: number; kind: 'optimize'; docText: string; mode: number }
  | { id: number; kind: 'specBuildCP'; spec: string; mode: number }
  | { id: number; kind: 'exportV5'; spec: string; mode: number };
  ...
  try {
    const result =
      msg.kind === 'optimize'
        ...
        : msg.kind === 'specBuildCP'
          ? await specBuildCreasePattern(msg.spec, msg.mode)
          : await exportV5(msg.spec, msg.mode);
